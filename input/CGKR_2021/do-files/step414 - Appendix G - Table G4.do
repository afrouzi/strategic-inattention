...
foreach var in dn dk dp dw {
	reg  FE_`var'_3m rev_epi_12m_exper ///
		if w8_exper_groupN==2 | w8_exper_groupN==1,  robust
	if "`var'"=="dn" {
		outreg2 using TableG4_cont_treatment2.dta, ///
			dta replace dec(3) ctitle("`var'")  nocons
	}
	else {
	outreg2 using TableG4_cont_treatment2.dta, ///	
		dta append dec(3) ctitle("`var'")  nocons
	}	
}
...
foreach var in dn dk dp dw {
	reg  FE_`var'_3m  rev_epi_12m_exper ///
		if w8_exper_groupN==3 | w8_exper_groupN==1, robust
	if "`var'"=="dn" {
		outreg2 using TableG4_cont_treatment3.dta, ///
			dta replace dec(3) ctitle("`var'")  
	}
	else {
	outreg2 using TableG4_cont_treatment3.dta, ///	
		dta append dec(3) ctitle("`var'")  	
	}
...
foreach var in dn dk dp dw {
	reg  FE_`var'_3m rev_epi_12m_exper  ///
		if w8_exper_groupN==4 | w8_exper_groupN==1, robust
 	if "`var'"=="dn" {
		outreg2 using TableG4_cont_treatment4.dta, ///
			dta replace dec(3) ctitle("dummy: `var'")  
	}
	else {
	outreg2 using TableG4_cont_treatment4.dta, ///	
		dta append dec(3) ctitle("cont: `var'")  
	}	
}
...
foreach var in dn dk dp dw {
	reg   FE_`var'_3m  rev_epi_12m_exper  ///
		if w8_exper_groupN==5 | w8_exper_groupN==1, robust
 	if "`var'"=="dn" {
		outreg2 using TableG4_cont_treatment5.dta, ///
			dta replace dec(3) ctitle("`var'") 
	}
	else {
	outreg2 using TableG4_cont_treatment5.dta, ///	
