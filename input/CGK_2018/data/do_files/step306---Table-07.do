...
postfile `1' str20 varx ///
	...
	using Table07, replace every(1)
		
capture drop t0
gen t0=_n-10

quiet foreach var in ///
			rev_sales_6m rev_dw_6m rev_dmc_6m rev_dk_6m rev_dn_6m rev_dp_main_6m ///
			rev_epi_12m_exper rev_epi_5y_exper rev_eue_12m_exper rev_egdp_12m_exper ///
			rev_epi_12m rev_epi_5y  rev_eue_12m  rev_egdp_12m rev_RBNZ_target_6m ///
			{
...
forvalues i1=1(1)8 {
	capture drop bb`i1'
	predict bb`i1', dfbeta(yT_`i1')
	replace flag=0 if abs(bb`i1')>0.5
}	

reg `var'  nT_* yT_* [aw=w7_wgtE_43] if flag==1, nocons robust cluster(w7_anzsic_code3)

lincom (yT_4+yT_5+yT_6+yT_7+yT_8)/5
...
reg `var'  nT_* yT_*  ///
	...
	w6_competitors i.w7_anzsic_code1
capture drop flag
gen flag=1
...
reg `var'  nT_* yT_*  ///
	logCU_w lnL_w abs_slope_w log_age w6_labor_cost_share ///
	w6_competitors i.w7_anzsic_code1 [aw=w7_wgtE_43] if flag==1, ///
